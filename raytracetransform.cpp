    ...
    )
{
    ...
    cameraToWorld = Mat44f(0.931056, 0, 0.364877, 0, 0.177666, 0.873446, -0.45335, 0, -0.3187, 0.48692, 0.813227, 0, -41.229214, 81.862351, 112.456908, 1);
...
int main()
{
    ...
    Mat44f objectToWorld = Mat44f(1.624241, 0, 2.522269, 0, 0, 3, 0, 0, -2.522269, 0, 1.624241, 0, 0, 0, 0, 1); // Matrix44f::kIdentity; 
    TriangleMash *mesh = loadPolyMeshFromFile("./teapot.geo", objectToWorld);

    objects.push_back(unique_ptr<Object>(mesh));

    objectToWorld = Mat44f(1.624241, 0, 2.522269, 0,
                           0,        3,        0, 0,
                           -2.522269, 0, 1.624241, 0,
                           5, 10, 10.5, 1);
    mesh = loadPolyMeshFromFile("./teapot.geo", objectToWorld);
