...
{
public:
    Sphere(Mat44f &ObjectToWorld, Vec3f& c, float &r): Object(ObjectToWorld), radius(r), radius2(r * r), center(c) {
        cerr << c << endl;
        cerr << r << endl;
    }
        ...
        switch (hitObject->type)
        {
            case kDiffuse:
                for (uint32_t i = 0; i < lights.size(); i++)
                {
                    ...
                    bool vis = !trace(hitPoint + hitNormal * bias, -lightDir, objects, t, index, uv, hitObject, kShadowRay);
                    ...
                    hitColor += vis * pattern * lightIntensity * std::max(0.f, hitNormal.dot(-lightDir));

#ifdef DEBUG
    //                cout << hitColor << " vis " << vis << " pattern " << pattern << " lightIntensity " << lightIntensity << endl;
#endif
    //exit(0);
                }
                break;
            case kReflection:
            {
                ...
                R.normalize();
                //hitColor += 0.8 * castRay(hitPoint + hitNormal * bias, R, objects, lights, depth + 1);
                break;
            }
            case kReflectionAndRefraction:
            {
                ...
                float kr;
#ifdef DEBUG
                cout << "dir " << dir << " hitNormal " << hitNormal << " ior " << hitObject->ior << endl;
#endif
                fresnel(dir, hitNormal, hitObject->ior, kr);

                bool outside = dir.dot(hitNormal) < 0;

                Vec3f b = bias * hitNormal;

                if (kr < 1)
                {
                    Vec3f refractionDirection = refract(dir, hitNormal, hitObject->ior).normalize();
                    Vec3f refractionRayOrig = outside ? hitPoint - b : hitPoint + b;
                    refractionColor = castRay(refractionRayOrig, refractionDirection, objects, lights, depth + 1);
                }

                Vec3f reflectionDirection = reflect(dir, hitNormal).normalize();
                Vec3f reflectionRayOrig = outside ? hitPoint + b : hitPoint - b;
                reflectionColor = castRay(reflectionRayOrig, reflectionDirection, objects, lights, depth + 1);

                hitColor += reflectionColor * kr + refractionColor * (1 - kr);
#ifdef DEBUG
                //if (kr < 1)
                {
                cout << hitColor << " reflectionColor " << reflectionColor << " refractionColor " << refractionColor << " kr " << kr << endl;
                exit(0);
                }
#endif
...
int main()
{
...
#if 1
    bias = 0.001;
    cameraToWorld = Mat44f(-0.972776, 0, -0.231748, 0, -0.114956, 0.8683, 0.482536, 0, 0.201227, 0.49604, -0.844661, 0, 6.696465, 22.721296, -30.097976, 1);

    TriangleMash *mesh = loadPolyMeshFromFile("geometry/backdrop.geo", kIdentity);

    if (mesh != nullptr)
    {
        mesh->type = kDiffuse;
        objects.push_back(unique_ptr<Object>(mesh));
        cerr << mesh->numTris << endl;
    }

    mesh = loadPolyMeshFromFile("geometry/cylinder.geo", kIdentity);
    ...
    mesh = loadPolyMeshFromFile("geometry/pen.geo", kIdentity);
    ...
    /* Light */
    Mat44f l2w(0.95292, 0.289503, 0.0901785, 0, -0.0960954, 0.5704, -0.815727, 0, -0.287593, 0.768656, 0.571365, 0, 0, 0, 0, 1);
    ...
    width = 1024;
    height = 747;
    cameraToWorld = Matrix44f(0.999945, 0, 0.0104718, 0, 0.00104703, 0.994989, -0.0999803, 0, -0.0104193, 0.0999858, 0.994934, 0, -0.978596, 17.911879, 75.483369, 1);
    
    TriangleMash *mesh = loadPolyMeshFromFile("geometry/glasses.geo", kIdentity);
    if (mesh != nullptr) {
        mesh->type = kReflectionAndRefraction;
        mesh->ior = 1.3;
    ...
    TriangleMash *mesh1 = loadPolyMeshFromFile("geometry/backdrop1.geo", kIdentity);
    if (mesh1 != nullptr) {
        mesh1->type = kDiffuse;
        mesh1->albedo = 0.18;
