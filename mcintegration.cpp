...
{
    r = X * XYZ_to_RGB[0][0] + Y * XYZ_to_RGB[0][1] + Z * XYZ_to_RGB[0][2];
    g = X * XYZ_to_RGB[1][0] + Y * XYZ_to_RGB[1][1] + Z * XYZ_to_RGB[1][2];
    b = X * XYZ_to_RGB[2][0] + Y * XYZ_to_RGB[2][1] + Z * XYZ_to_RGB[2][2];
...
float linerp(const float *f, const short &i, const float &t, const int &max)
{
    return f[i] * (1 - t) + f[min(max, i + 1)] * t;
}
...
int main()
{
    const uint32_t squareSize = 64;
    const uint32_t width = squareSize * 6, height = squareSize * 4;
    ...
    while (1)
    {
        ...
        {
            short row = (y / squareSize);

            for (int x = 0; x < width; x++, pixIndex++)
            {
                short column = (x / squareSize);
    ...
    // save image to file
    std::ofstream ofs;
    ofs.open("mcintegration.ppm", std::ios::out | std::ios::binary);
    ofs << "P6\n" << width << " " << height << "\n255\n";
    for (uint32_t i = 0; i < width * height; ++i) {
        unsigned char r = (unsigned char)(255 * std::min(1.0f, pixels[i * 3]));
        unsigned char g = (unsigned char)(255 * std::min(1.0f, pixels[i * 3 + 1]));
        unsigned char b = (unsigned char)(255 * std::min(1.0f, pixels[i * 3 + 2]));
        ofs << r << g << b;
    }
