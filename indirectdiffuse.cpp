        ...
        switch (hitObject->type)
        {
            case kDiffuse:
            {
                ...
                for (uint32_t i = 0; i < lights.size(); i++)
                {
                    ...
                    directLighting += vis * lightIntensity * max(0.f, hitNormal.dot(-lightDir));
                }
...
int main()
{
    ...
    width = 512;
    height = 512;
    cameraToWorld = Matrix44f(0.965926, 0, -0.258819, 0, 0.0066019, 0.999675, 0.0246386, 0, 0.258735, -0.0255078, 0.965612, 0, 0.764985, 0.791882, 5.868275, 1);
    
    TriangleMash *mesh = loadPolyMeshFromFile("geometry/planegi.geo", kIdentity);
    if (mesh != nullptr) {
        mesh->albedo = Vec3f(0.225, 0.144, 0.144);
        objects.push_back(std::unique_ptr<Object>(mesh));
    }

    mesh = loadPolyMeshFromFile("geometry/cubegi.geo", kIdentity);
    if (mesh != nullptr) {
        mesh->albedo = Vec3f(0.188559, 0.287, 0.200726);
        objects.push_back(std::unique_ptr<Object>(mesh));
    }
...
//    lights.push_back(std::unique_ptr<Light>(new DistantLight(l2w, 1, 16)));
    ...
    cerr << mesh->numTris << " " << passedTime / 1000 << endl;
}
